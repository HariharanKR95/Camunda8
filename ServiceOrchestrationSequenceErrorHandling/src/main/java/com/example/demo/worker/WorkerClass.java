...
//			errmssg.put("errormessage", "I/O error for post method "+url+endPoint+" :"+e.getMessage());
//			errmssg.put("Error occurred",endPoint);
//			errmssg.put("API name", url+endPoint);
//			zeebeclient.newSetVariablesCommand(job.getProcessInstanceKey()).variables(errmssg).send().join();
			Map<String, Object> errmssg = new HashMap<>();
			...
			zeebeclient.newSetVariablesCommand(job.getProcessInstanceKey()).variables(finalResponse).send().join();
			 ...
			 client.newThrowErrorCommand(job.getKey()).errorCode("Error_Occurred").errorMessage("Error_Occurred in employe job").send().join();
	...
	public Object addressJsonWorker(final JobClient client, final ActivatedJob job) {
		try {
			...
			zeebeclient.newSetVariablesCommand(job.getProcessInstanceKey()).variables(finalResponse).send().join();
			 ...
			 client.newThrowErrorCommand(job.getKey()).errorCode("Error_Occurred").errorMessage("Error_Occurred in address job").send().join();
			//client.newCompleteCommand(job.getKey()).variables(finalResponse).send().join();
		}
		return null;
	...
	public Object idJsonWorker(final JobClient client, final ActivatedJob job) {
		try {
			...
			zeebeclient.newSetVariablesCommand(job.getProcessInstanceKey()).variables(finalResponse).send().join();
			client.newThrowErrorCommand(job.getKey()).errorCode("Error_Occurred").errorMessage("Error_Occurred in id job").send().join();
			//client.newCompleteCommand(job.getKey()).variables(finalResponse).send().join();
		}
		return null;
	}
...
//			});	
			...
			zeebeclient.newSetVariablesCommand(job.getProcessInstanceKey()).variables(finalResponse).send().join();
			 ...
			 client.newThrowErrorCommand(job.getKey()).errorCode("Error_Occurred").errorMessage("Error_Occurred in all job").send().join();
			//client.newCompleteCommand(job.getKey()).variables(finalResponse).send().join();
